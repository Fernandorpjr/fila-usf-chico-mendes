...
1. **SuperDoc extension installed.** Verify:
    ```bash
    code --list-extensions | grep -i superdoc
    ```

2. **Document must be open** in VS Code before editing:
    ```bash
    code path/to/doc.docx
    ```
...
- The raw command JSON back → document not open, re-open with `code path/to/doc.docx`
...
**Why:** With track changes enabled, plain `getText` returns both deleted AND inserted text concatenated (e.g., `"LawvableJamie"` instead of `"Jamie"`). Use `resolveTrackedChanges: true` to get the current version as if all changes were accepted (see `getText` docs below).
...
// Get text with tracked changes resolved (deleted text excluded, insertions kept)
{"command":"getText","args":{"format":"text","resolveTrackedChanges":true}}

// Get first 2000 chars (like head)
{"command":"getText","args":{"format":"text","head":2000}}

// Get last 1000 chars (like tail)
{"command":"getText","args":{"format":"text","tail":1000}}
```

**Formats:** `text` (plain text with paragraph breaks), `html` (full HTML), `both` (default)

**`resolveTrackedChanges`** (optional, default `false`): When `true`, returns text as if all tracked changes were accepted — deleted text is excluded, inserted text is kept. Use this when you need the current version of the document without struck-through content confusing the output. Only affects `text` format.

**`head` / `tail`** (optional, mutually exclusive): Truncate text output to the first or last N characters. Useful for large documents when you only need the beginning or end. A truncation notice with remaining char count is included. Only affects `text` format.
...
**Note:** After accepting/rejecting, `getText` returns clean text. To preview the resolved text *before* accepting, use `resolveTrackedChanges: true`.
...
- **Command not executing / raw JSON returned**: Document not open. Re-open with `code --profile path/to/doc.docx` and retry.
